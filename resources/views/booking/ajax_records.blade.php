    ...
    @if (isset($data['records']) && count($data['records'])>0)
      @foreach ($data['records'] as $key => $item)
        ...
        <tr>
          ...
          <td>
            <strong class="font-sm">Check In: </strong>{{ $item->booking_logs[0]->checkin_date }}<br>
            <strong class="font-sm">Check Out: </strong>{{ $item->booking_logs[0]->checkout_date }}
          </td>
          <td>
          ...
          <td>
            ...
            <strong class="font-sm">Admin Charges: </strong>{{ default_value($item->booking_logs[0]->admin_charges, "num") }}<br>
            <strong class="font-sm">Security Charges: </strong>{{ default_value($item->booking_logs[0]->security_charges, "num") }}
          </td>
          ...
          <td>
            ...
            <button type="button" class="btn btn-warning btn-sm checkin_btn mb-3" title="Add Other Charges" data-item_id="{{$item->id}}" data-metadata="porp={{$item->booking_logs[0]->properties->id}},resu={{$item->booked_for_user->id}},latot={{$item->total_payable}}"  data-toggle="modal" data-target="#"><i class="fa fa-navicon"></i> </button>

            <button type="button" class="btn btn-purple btn-sm checkin_btn mb-3" title="Check In" data-item_id="{{$item->id}}" data-metadata="porp={{$item->booking_logs[0]->properties->id}},resu={{$item->booked_for_user->id}},latot={{$item->total_payable}}"  data-toggle="modal" data-target="#"><i class="fa fa-calendar-plus-o"></i> </button>

            <button type="button" class="btn btn-success btn-sm initial_deposit_btn mb-3" title="Add Payment" data-total="{{$item->total_payable}}" data-item_id="{{$item->id}}" data-metadata="porp={{$item->booking_logs[0]->properties->id}},resu={{$item->booked_for_user->id}}"  data-toggle="modal" data-target="#"><i class="fa fa-money"></i> </button>
          </td>
        </tr>
      @endforeach
    @endif
